...
// Observação importante: Sempre que for compilar o programa, compile estando no arquivo main ou então não vai funcionar
using namespace std;
...
int main()
{
    ...
    cin >> sMoeda;
    cout << "Digite quanto você ganha em " << sMoeda << endl;
    cin >> ganhos;
    cout << "O valor que você ganha em reais é: R$ " << CotacaoMoedas(sMoeda, ganhos) << endl;
    ...
    cout << "DeclaraÃ§Ã£o Simplificada: " << endl;
...
float retornaLegal(float* ptr, float* ptr2) {
    float total;
    cout << "Rendimento tributÃ¡vel anual: " << *ptr << endl;
    cout << "Despesas dedutÃ­veis: " << *ptr2 << endl;
    total = *ptr - *ptr2;
    cout << "DEDUÃ‡ÃƒO: " << total << endl << endl;
    return total;
}
...
float getValor(string msg)
{
    ...
    do {
        ...
        cin >> valor;
        if (valor < 0) {
            cout << "Valor invÃ¡lido (deve ser um nÃºmero maior ou igual a zero)." << endl;
        }
    } while (valor < 0);
    return valor;
}
...
float despesasDedutiveis() {
    ...
    cout << "Digite o valor da contribuiÃ§Ã£o previdenciÃ¡ria oficial: ";
    cin >> inss;
    ...
    sn = toupper(sn);
    if (sn == 'S') {
        cout << "Insira o nÃºmero de dependentes: ";
        cin >> qnt_dependente;
        maiorOuIgualaZero(qnt_dependente);
        desconto += (qnt_dependente * 2275.08f);
    }

    cout << "Realiza pagamento de pensÃ£o alimentÃ­cia? [S/N] ";
    cin >> sn;
    sn = toupper(sn);
    if (sn == 'S') {
    ...
    cout << "Possui gastos com educaÃ§Ã£o? [S/N] ";
    cin >> sn;
    sn = toupper(sn);
    if (sn == 'S') {
    ...
    cout << "Possui gastos com saÃºde? [S/N] ";
    cin >> sn;
    sn = toupper(sn);
    if (sn == 'S') {
        cout << "Digite o valor dos gastos: ";
...
void chamarImposto(float *ptr) {
    cout << "Imposto a pagar incidente nos rendimentos tributÃ¡veis: " << *ptr;
